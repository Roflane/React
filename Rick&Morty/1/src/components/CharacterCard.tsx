    ...
    >
      ...
      <div className="p-5">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={
              `text-sm px-2 py-1 rounded-full ${status === 'Alive' ?
                  'bg-green-500/20 text-green-400' : status === 'Dead' ?
                      'bg-red-500/20 text-red-400' : 'bg-gray-500/20 text-gray-400'}`}>
            {status}
          </span>
          <span className={`text-xl font-bold text-green-400 mb-2 group-hover:text-purple-400 transition-colors`}> {counter} </span>
        </div>
      </div>
    </div>
  )
